...
pub fn part1(program: &[i64]) -> Option<i64> {
    execute(program, &[1]).and_then(|output| output.iter().last().cloned())
}

pub fn part2(program: &[i64]) -> Option<i64> {
    execute(program, &[2]).and_then(|output| output.iter().last().cloned())
}

fn execute(program: &[i64], input: &[i64]) -> Option<(Vec<i64>)> {
    let mut base = 0;
    let mut instr_ptr = 0;
    ...
    loop {
        match memory.get(&instr_ptr).map(|&i| parse_instr(i)) {
            ...
            Some((IN_OP, mode, _, _)) => {
                ...
                set_value(&mut memory, addr, input_iter.next()?);
                instr_ptr += 2;
            }
